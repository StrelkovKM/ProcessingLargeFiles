...
3. В директории `src` в файле `main.cpp` в строку:
   ...
   Констурктор <ins>самостоятельно запрашивает сколько свободной RAM есть у компьютера</ins>, если нужно ее задать вручную использовать метод SetRAMSize, аргументом которого является объем памяти в байтах.
   Example:
   ```cpp
